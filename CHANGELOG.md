...
- 看板顶栏新增版本更新提示：检测到新版本时图标上显示红点，点击弹窗显示版本信息和一键升级入口

### 修复

- README 致谢文案改为「学AI，上L站」
...
- **一键升级后服务仍运行旧版本且无提示**的问题：
  - 升级前先执行新二进制的 `-version` 校验（下载不完整/架构不符/文件损坏时直接报错，不再替换线上文件）
  - 「检查更新」探测磁盘二进制版本：发现「新版本文件已就位但进程未重启」时，页面明确提示手动重启命令，而不是再次引导升级
  - 升级轮询超时文案改为指向「检查更新」查看处理提示
